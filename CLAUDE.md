...
13. `mq()` は**対象セレクタの内側・基本宣言の直後**に書く（要素のスタイルを1箇所に集める）。例外：1つのブレークポイントで複数の子要素のレイアウト関係が同時に変わる場合（グリッドの組み替え等）のみ、親レベルでまとめてよい（要コメント）
14. セレクタは `&` 入れ子記法で統一する（`.p-header { &__inner {} }`）。既存のフラット記法ファイル（_p-header・_p-footer等）は**次にそのファイルを触るタイミング**で統一し、統一だけを目的とした一括改修はしない。状態組合せが複雑なファイルは冒頭コメントにクラス一覧を書いて検索性を担保する
